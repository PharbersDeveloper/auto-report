...
object DotMn {
    def apply(args: MapArgs): DotMn = new DotMn(args)
}

//DOT 百万单位
class DotMn(override val defaultArgs: pActionArgs) extends pActionTrait {
    ...
    override def perform(pr: pActionArgs): pActionArgs = {
        ...
        val result = CalculationUtils(sourceDF, ymDF, displayName).getSum() / 1000000
        StringArgs(result.toString)
    }
}
