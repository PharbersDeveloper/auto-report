package com.pharbers.report.lilly.format
...
case class sourceFileFormat() {
    ...
    def allTableFormat(bigTableDF: DataFrame, DotMappingDF: DataFrame): DataFrame = {
        val configDF = DotMappingDF.withColumnRenamed("PRODUCT DESC", "PRODUCT DESC_config")
            .withColumnRenamed("PACK DESC", "PACK DESC_config")
            .filter(col("ADD RATE") =!= "N")
        val resultDF = bigTableDF.join(configDF, bigTableDF("PRODUCT DESC") === configDF("PRODUCT DESC_config") && bigTableDF("PACK DESC") === configDF("PACK DESC_config"))
